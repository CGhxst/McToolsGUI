...
namespace McCrypt
{
    public class Marketplace
    {
        ...
        private static string[] dontEncrypt = new string[] { "manifest.json", "contents.json", "texts", "pack_icon.png"};
        ...
        private static byte[] worldOrContentsJsonDecrypt(string file, string productType)
        {
            ...
            { // Open file for reading
                ...
                { 
                    ...
                    if (version == 0 && magic == 0x9bcfb9fc) // is valid header?
                    {
                        ...
                        if (key == null)
                        {
                            // All skin packs and persona uses the same key
                            if (productType == "skin_packs" || productType == "persona")
                            {
                                key = Encoding.UTF8.GetBytes("s5s5ejuDru4uchuF2drUFuthaspAbepE");
                            }
                            else
                            {
                                throw new Exception("key not found, make sure to be logged in on the account that owns the item");
                            }
                        }
        ...
        // Mutli-Threaded.
        public static void DecryptContents(string contentsPath, string productType)
        {
            ...
            {
                byte[] decryptedData = worldOrContentsJsonDecrypt(oldSchoolZipe, productType); // Decrypt the zipe file
            ...
            else if (File.Exists(contentsJsonPath)) // Resource packs or Skin Packs
            {
                ...
                byte[] decryptedData = worldOrContentsJsonDecrypt(contentsJsonPath, productType); // Decrypt the contents.json file
                ...
                if (Directory.Exists(subPacksFolder))
                {
                    ...
                    foreach (string subPack in subPacks)
                        DecryptContents(Path.Combine(subPacksFolder, subPack), productType);
                }
            }
            else // World Templates
            {
                ...
                if (Directory.Exists(behaviourPacksFolder))
                {
                    ...
                    foreach (string behaviourPack in behaviourPacks)
                        DecryptContents(Path.Combine(behaviourPacksFolder, behaviourPack), productType);
                }
                ...
                if (Directory.Exists(resourcePacksFolder))
                {
                    ...
                    foreach (string resourcePack in resourcePacks)
                        DecryptContents(Path.Combine(resourcePacksFolder, resourcePack), productType);
                }
                ...
                if (Directory.Exists(levelDbFolder))
                {
                    ...
                    foreach (string levelDbFile in levelDbFiles)
                    {
                        Thread thrd = new Thread(() =>
                        {
                            ...
                            try
                            {
                                decryptedData = worldOrContentsJsonDecrypt(fileToDecrypt, productType); // Decrypt file
